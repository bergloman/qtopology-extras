...
describe.only('NN', function () {
    describe('simple', function () {
        ...
        it('add single and test distances', function () {
            let target = new nn.NN({});
            target.add({ a: 2, b: 3 });
            assert.strictEqual(target.getDistance({ a: 2, b: 3 }), 0);
            assert.strictEqual(target.getDistance({ c: 2, d: 3 }), 1);
            tt.approxEqual(target.getDistance({ a: 2, c: 3 }), 0.69230769230, 0.000001);
        });
        it('add single and test distances', function () {
            let target = new nn.NN({});
            target.add({ a: 1, b: 1 });
            console.log(target.getDistance({ a: 1, c: 1 }));
            // assert.strictEqual(target.getDistance({ a: 1, c: 1 }), 0);
            // tt.approxEqual(target.getDistance({ a: 2, c: 3 }), 0.69230769230, 0.000001);
        });
        it('add single and test distances, under min_len', function () {
            let target = new nn.NN({ min_len: 5 });
            target.add({ a: 2, b: 3 });
            assert.strictEqual(target.getDistance({ a: 2, b: 3 }), -1);
        });
    });
    describe('complex', function () {
        it.only('add three', function () {
            let target = new nn.NN({ k: 2 });
            target.add({ a: 1 });
            target.add({ b: 2 });
            target.add({ c: 3 });
            assert.strictEqual(target.getDistance({ a: 2, d: 3 }), 1);
        });
        ...
        it('add three and test distances', function () {
            ...
            target.add({ b: 2 });
            target.add({ c: 3 });
            assert.strictEqual(target.getDistance({ a: 2 }), 0);
            assert.strictEqual(target.getDistance({ b: 3 }), 0);
            assert.strictEqual(target.getDistance({ c: 4 }), 0);
        ...
        it('add three with auto_add, without min_len', function () {
            let target = new nn.NN();
            assert.strictEqual(target.getDistance({ a: 1 }, true), 0);
            assert.strictEqual(target.getDistance({ b: 2 }, true), 1);
            assert.strictEqual(target.getDistance({ c: 3 }, true), 1);
            tt.approxEqual(target.getDistance({ a: 1, b: 1, c: 1 }), 0.4226497308103, 0.000001);
        });
        it('add three with auto_add, with min_len', function () {
            let target = new nn.NN({ min_len: 2 });
            assert.strictEqual(target.getDistance({ a: 1 }, true), -1);
            assert.strictEqual(target.getDistance({ b: 2 }, true), -1);
            assert.strictEqual(target.getDistance({ c: 3 }, true), 1);
            tt.approxEqual(target.getDistance({ a: 1, b: 1, c: 1 }), 0.4226497308103, 0.000001);
        });
        ...
        it.only('bugifxing 1', function () {
            ...
            assert.strictEqual(target.getDistance(obj.names, true), -1);
            tt.approxEqual(target.getDistance({ a: 1, b: 1, c: 1 }), 0.4226497308103, 0.000001);
        });
    });
});
